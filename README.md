...
一個輕量的浮動小工具，即時顯示 Claude Code 的用量限制狀態，支援 **macOS** 與 **Windows**。
...
- **系統匣 / 選單列整合**：關閉視窗後縮到系統匣（Windows）或 macOS 選單列，可重新顯示
- **置頂釘選**：可切換視窗是否常駐最上層
...
- **macOS** 10.15 Catalina 以上，或 **Windows** 10 / 11
...
| 平台 | 檔案 |
|------|------|
| macOS | `ClaudeMonitor-macOS.zip` — 解壓後將 `.app` 拖入 `/Applications` |
| Windows | `ClaudeMonitor.exe` — 直接執行 |

> **macOS 首次執行提示**：若出現「無法驗證開發者」警告，請在 Finder 中對 `.app` 按右鍵 → 「打開」。
...
| `~/.claude/claude_monitor.lock` | 單一實例鎖定檔（macOS / Linux） |
...
- **標題列雙擊**：將視窗重置回右上角預設位置
- **📌 按鈕**：切換視窗置頂（亮色 = 啟用）
- **✕ 按鈕**：縮小至系統匣 / 選單列（有安裝 pystray）或關閉程式
- **時間戳記點擊**：手動觸發立即更新
- **邊緣 / 角落拖曳**：調整視窗大小
...
- 用量資料每 5 分鐘自動更新一次；遇到速率限制時會自動退避重試
